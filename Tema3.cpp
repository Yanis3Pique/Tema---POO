...
public:
    Echipa(const string& name = "FC Arges", const Antrenor<int>& coach = Antrenor<int>(), const Stadion& stadium = Stadion())
            : name(name), coach(make_unique<Antrenor<int>>(coach)), stadium(stadium) {created_teams.push_back(*this);}
    ...
    bool hasName(const string& teamName) const {return name == teamName;}

    // Iterator design pattern
    class Iterator : public std::iterator<std::input_iterator_tag, Echipa> {
    private:
        typename list<Echipa>::iterator it;
    public:
        explicit Iterator(typename list<Echipa>::iterator iterator) : it(iterator) {}
        Iterator& operator++() {
            ++it;
            return *this;
        }
        ...
        Echipa& operator*() {return *it;}
    };
    Iterator begin() {return Iterator(created_teams.begin());}
    Iterator end() {return Iterator(created_teams.end());}
    static void addEchipa(Echipa echipa) { created_teams.push_back(echipa); }
};
...
Echipa* Echipa::getTeamByName(const string& name) {
    ...
    auto it = find_if(created_teams.begin(), created_teams.end(), [&](const Echipa& team) {
        // Convert the team name to lowercase for case-insensitive comparison
        string lowercaseTeamName = team.getName();
        transform(lowercaseTeamName.begin(), lowercaseTeamName.end(), lowercaseTeamName.begin(), ::tolower);

        return lowercaseTeamName == lowercaseName;
    });
...
int main() {
    Echipa team1("FC Arges", Antrenor("John Doe", 45, 3000, 3), Stadion("Municipal", 20000));
    Echipa team2("FC Dinamo", Antrenor("Jane Smith", 50, 4000, 4), Stadion("Dinamo", 25000));
    Echipa team3("FC Steaua", Antrenor("Mike Johnson", 55, 4500, 4), Stadion("National Arena", 55000));

    cout << "Echipe create:" << endl; // Iteram folosind design pattern-ul iterator scris de noi
    for (Echipa::Iterator it = team1.begin(); it != team1.end(); ++it) {
        cout << *it << endl;
    }

    cout << "------------------------------------------------------------------------------------------------------------------------" << endl << endl;
    Echipa::Iterator it = team1.begin(); // Loop normal cu iterator
    Echipa::Iterator end = team1.end();
    for (; it != end; ++it) {
        cout << *it << endl;
    }
    ...
    Echipa team("Barcelona"), echipa22("Madrid", antrenor1, s1), echipa33("Bilbao", antrenor3, s2);
    Echipa::addEchipa(team);
    Echipa::addEchipa(echipa22);
    Echipa::addEchipa(echipa33);
